...
function segmentosDia(f, h) {
  const ini = nHora(h.horaInicio) ?? 540, fin = nHora(h.horaFin) ?? 1200
  const pi = nHora(h.pausaInicio), pf = nHora(h.pausaFin)
  if (pi !== null && pf !== null && pi > ini && pf > pi && pf < fin) return [[conHora(f, ini), conHora(f, pi)], [conHora(f, pf), conHora(f, fin)]]
...
export default function AgendaMecanicosPage() {
  ...
  const vehiculosFiltrados = useMemo(() => {
    const q = normalizar(busqueda); const lista = q ? vehiculos.filter(v => normalizar(`${v.matricula} ${v.numeroOR} ${v.cliente} ${v.marcaModelo}`).includes(q)) : vehiculos
    return lista.slice(0, 80)
  }, [vehiculos, busqueda])
  const resumen = useMemo(() => ({ total: trabajos.length, pendientes: trabajos.filter(t => !t.estado || t.estado === "pendiente").length, enProceso: trabajos.filter(t => t.estado === "en_proceso").length, terminados: trabajos.filter(t => t.estado === "terminado").length }), [trabajos])
  ...
  async function agregarMecanico(e) {
    e.preventDefault(); const nom = nombre.trim(); if (!nom) return
    setGuardando(true)
  ...
  async function asignarTrabajo(e) {
    e.preventDefault(); const mecanico = mecanicosActivos.find(m => m.id === form.mecanicoId)
    ...
    const duplicado = trabajos.find(t => t.estado !== "terminado" && mismoVehiculo(t, vehiculo)); if (duplicado) return mostrar("aviso", "Vehiculo ya asignado", `Este vehiculo ya esta asignado a ${duplicado.mecanicoNombre || "un mecanico"}. No se ha duplicado.`)
    setGuardando(true)
    try { const inicio = form.fechaInicio ? new Date(form.fechaInicio) : new Date(); const horas = Number(form.duracionHoras||0); const horario = horarioOk(mecanico.horario)
      ...
      setForm(f => ({ ...f, notas:"", manualMatricula:"", manualNumeroOR:"", manualCliente:"", manualMarcaModelo:"" })); await cargarDatos(); mostrar("ok", "Vehiculo asignado", `Finalizacion segun horario: ${fechaTexto(calcularFinLaboral(inicio, horas, horario))}.`)
    } catch (e) { console.error(e); mostrar("error", "Error", "No se ha podido asignar el vehiculo.") } finally { setGuardando(false) }
  }
