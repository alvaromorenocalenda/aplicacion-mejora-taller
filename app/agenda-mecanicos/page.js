...
const horarioOk = (h) => ({ ...DEFAULT_HORARIO, ...(h || {}) })
...
const horarioTxt = (h) => { const x = horarioOk(h); return `${x.horaInicio}-${x.horaFin} · Pausa ${x.pausaInicio}-${x.pausaFin}${x.sabado ? " · Sab" : ""}${x.domingo ? " · Dom" : ""}` }
const mismoVehiculo = (t, v) => {
  if (t?.vehiculoId && v?.id && String(t.vehiculoId) === String(v.id)) return true
  const a = normalizar(`${t?.matricula || ""}${t?.numeroOR || ""}`)
  const b = normalizar(`${v?.matricula || ""}${v?.numeroOR || ""}`)
  return !!a && !!b && a === b
}
...
function siguienteDia(f, h) {
  ...
  x.setHours(0, 0, 0, 0)
  for (let i = 0; i < 370; i++) {
    if (esLaborable(x, h)) return conHora(x, nHora(h.horaInicio) ?? 540)
    x.setDate(x.getDate() + 1)
  }
  return x
}
function segmentosDia(f, h) {
  const ini = nHora(h.horaInicio) ?? 540
  const fin = nHora(h.horaFin) ?? 1200
  const pi = nHora(h.pausaInicio)
  const pf = nHora(h.pausaFin)
  if (pi !== null && pf !== null && pi > ini && pf > pi && pf < fin) {
    return [[conHora(f, ini), conHora(f, pi)], [conHora(f, pf), conHora(f, fin)]]
  }
  return [[conHora(f, ini), conHora(f, fin)]]
}
function calcularFinLaboral(inicio, horas, base) {
  const h = horarioOk(base)
  let actual = toDate(inicio) || new Date()
  let restante = Math.round(Number(horas || 0) * 60)
  if (!restante) return null
  for (let i = 0; i < 20000; i++) {
    if (!esLaborable(actual, h)) { actual = siguienteDia(actual, h); continue }
    const segmentos = segmentosDia(actual, h)
    const finDia = segmentos[segmentos.length - 1][1]
    if (actual >= finDia) { actual = siguienteDia(actual, h); continue }
    let avanzo = false
...
export default function AgendaMecanicosPage() {
  ...
  async function cargarDatos() {
    try {
      ...
      mecSnap.docs.forEach((d) => {
        const x = d.data() || {}
        const nom = x.nombre || x.displayName || x.email || "Mecanico"
        if (x.activo === false) {
          ocultos.add(d.id)
          ocultos.add(x.uid || "")
          ocultos.add(normalizar(nom))
          return
        }
      ...
      usersSnap.docs.forEach((d) => {
        const x = d.data() || {}
        const rol = String(x.rol || x.role || "").toUpperCase()
        const nom = x.nombre || x.displayName || x.name || x.email || "Mecanico"
        if (rol.includes("MECANICO") && !ocultos.has(d.id) && !ocultos.has(normalizar(nom)) && !mapa.has(d.id)) {
          mapa.set(d.id, { id: d.id, nombre: nom, activo: x.activo !== false, uid: d.id, horario: horarioOk(x.horario), origen: "usuario" })
        }
      })

      MECANICOS_BASE.forEach((nom) => {
        const id = `base-${normalizar(nom)}`
        if (!ocultos.has(id) && !ocultos.has(normalizar(nom)) && !Array.from(mapa.values()).some((m) => normalizar(m.nombre) === normalizar(nom))) {
          mapa.set(id, { id, nombre: nom, activo: true, horario: DEFAULT_HORARIO, origen: "base" })
        }
      })

      const listaVehiculos = vehSnap.docs.map((d) => {
        const raw = d.data() || {}
        const x = raw.datos || raw || {}
        const creado = raw.creadoEn || raw.createdAt || x.creadoEn || x.createdAt || null
        const creadoMs = creado?.toMillis?.() || (creado?.seconds ? creado.seconds * 1000 : 0)
        return {
          id: d.id,
          matricula: x.matricula || "",
          numeroOR: x.numeroOR || "",
          cliente: x.nombreCliente || x.nombre || "",
          marcaModelo: x.marcaModelo || x.modelo || "",
          estadoPresupuesto: raw.estadoPresupuesto || x.estadoPresupuesto || "",
          creadoMs,
        }
      }).filter((v) => !["FINALIZADO", "DENEGADO"].includes(String(v.estadoPresupuesto || "").toUpperCase()))
  ...
  async function agregarMecanico(e) {
    ...
    setGuardando(true)
    try {
      await addDoc(collection(db, "mecanicos"), { nombre: nom, activo: true, horario: DEFAULT_HORARIO, creadoEn: serverTimestamp() })
      setNombre("")
      await cargarDatos()
      mostrar("ok", "Mecanico añadido", `${nom} se ha añadido a la agenda.`)
    } catch (e) {
      console.error(e)
      mostrar("error", "Error", "No se ha podido añadir el mecanico.")
    } finally { setGuardando(false) }
  }

  function cambiarHorarioLocal(id, campo, valor) {
    setMecanicos((prev) => prev.map((m) => m.id === id ? { ...m, horario: { ...horarioOk(m.horario), [campo]: valor } } : m))
  }
  async function guardarHorario(m) {
    await setDoc(doc(db, "mecanicos", m.id), { nombre: m.nombre, activo: true, horario: horarioOk(m.horario), actualizadoEn: serverTimestamp() }, { merge: true })
    await cargarDatos()
    mostrar("ok", "Horario guardado", `Horario actualizado para ${m.nombre}.`)
  }
  async function quitarMecanico(m) {
    if (!confirm("Quieres quitar este mecanico de la agenda?")) return
    await setDoc(doc(db, "mecanicos", m.id), { nombre: m.nombre, activo: false, uid: m.uid || m.id, actualizadoEn: serverTimestamp() }, { merge: true })
    await cargarDatos()
  }
  ...
  async function asignarTrabajo(e) {
    ...
    try {
      ...
      await cargarDatos()
      mostrar("ok", "Vehiculo asignado", `Finalizacion segun horario: ${fechaTexto(calcularFinLaboral(inicio, horas, horario))}.`)
    } catch (e) {
      console.error(e)
      mostrar("error", "Error", "No se ha podido asignar el vehiculo.")
    } finally { setGuardando(false) }
  }
  async function cambiarEstado(id, estado) {
    if (estado === "terminado") mostrar("ok", "Trabajo terminado", "Trabajo marcado como terminado.")
  ...
  return <main className="min-h-screen bg-gray-100 p-6">
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-5 no-print">
        <form onSubmit={agregarMecanico} className="bg-white rounded-xl shadow p-5 space-y-4"><h2 className="text-xl font-bold">Añadir mecanico</h2><input value={nombre} onChange={(e) => setNombre(e.target.value)} className="w-full border rounded px-3 py-2" placeholder="Nombre del mecanico"/><button disabled={guardando} className="w-full bg-green-600 text-white px-4 py-2 rounded">+ Añadir mecanico</button><div className="pt-2 border-t space-y-3"><h3 className="font-semibold">Mecanicos activos</h3>{mecanicosActivos.length === 0 ? <p className="text-sm text-gray-500">Todavia no hay mecanicos.</p> : mecanicosActivos.map((m) => { const abierto = !!horariosAbiertos[m.id]; return <div key={m.id} className="bg-gray-50 rounded px-3 py-3 space-y-2"><div className="flex justify-between items-center gap-2"><b>{m.nombre}</b><div className="flex gap-2"><button type="button" onClick={() => setHorariosAbiertos((p) => ({ ...p, [m.id]: !p[m.id] }))} className="bg-purple-600 text-white text-xs px-3 py-1 rounded">Horario</button><button type="button" onClick={() => quitarMecanico(m)} className="text-red-600 text-sm">Quitar</button></div></div>{abierto && <div className="space-y-2 rounded border bg-white p-3"><div className="grid grid-cols-2 gap-2 text-xs"><label>Entrada<input type="time" value={horarioOk(m.horario).horaInicio} onChange={(e) => cambiarHorarioLocal(m.id, "horaInicio", e.target.value)} className="w-full border rounded px-2 py-1"/></label><label>Salida<input type="time" value={horarioOk(m.horario).horaFin} onChange={(e) => cambiarHorarioLocal(m.id, "horaFin", e.target.value)} className="w-full border rounded px-2 py-1"/></label><label>Pausa inicio<input type="time" value={horarioOk(m.horario).pausaInicio} onChange={(e) => cambiarHorarioLocal(m.id, "pausaInicio", e.target.value)} className="w-full border rounded px-2 py-1"/></label><label>Pausa fin<input type="time" value={horarioOk(m.horario).pausaFin} onChange={(e) => cambiarHorarioLocal(m.id, "pausaFin", e.target.value)} className="w-full border rounded px-2 py-1"/></label></div><div className="flex gap-3 text-xs"><label><input type="checkbox" checked={!!horarioOk(m.horario).sabado} onChange={(e) => cambiarHorarioLocal(m.id, "sabado", e.target.checked)}/> Sabado</label><label><input type="checkbox" checked={!!horarioOk(m.horario).domingo} onChange={(e) => cambiarHorarioLocal(m.id, "domingo", e.target.checked)}/> Domingo</label></div><button type="button" onClick={() => guardarHorario(m)} className="w-full bg-blue-700 text-white rounded px-3 py-1 text-sm">Guardar horario</button></div>}<p className="text-xs text-gray-500">{horarioTxt(m.horario)}</p></div>})}</div></form>
      ...
      <section className="bg-white rounded-xl shadow p-5 print-area"><div className="flex justify-between items-center mb-4"><div><h2 className="text-xl font-bold">Planificacion</h2><p className="text-sm text-gray-500 print:block hidden">Impreso el {new Date().toLocaleString("es-ES")}</p></div><div className="flex gap-2 no-print"><button onClick={imprimirPlanificacion} className="bg-green-600 text-white px-4 py-2 rounded">Imprimir planificacion</button><button onClick={cargarDatos} className="bg-gray-700 text-white px-4 py-2 rounded">Actualizar</button></div></div><div className="mb-5 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900"><strong>Recordatorio:</strong> cuando se finalice un trabajo, avisar al asesor por chat o presencialmente.</div><div className="print-summary grid grid-cols-2 md:grid-cols-4 gap-3 mb-5"><div className="bg-gray-50 border rounded p-3"><p className="text-xs text-gray-500">Trabajos</p><b>{resumen.total}</b></div><div className="bg-yellow-50 border rounded p-3"><p className="text-xs text-gray-500">Pendientes</p><b>{resumen.pendientes}</b></div><div className="bg-blue-50 border rounded p-3"><p className="text-xs text-gray-500">En proceso</p><b>{resumen.enProceso}</b></div><div className="bg-green-50 border rounded p-3"><p className="text-xs text-gray-500">Terminados</p><b>{resumen.terminados}</b></div></div><div className="grid grid-cols-1 xl:grid-cols-2 gap-5 print-grid">{mecanicosActivos.map((m) => { const lista = trabajos.filter((t) => t.mecanicoId === m.id); const horas = lista.reduce((a, t) => a + Number(t.duracionHoras || 0), 0); return <div key={m.id} className="border rounded-xl overflow-hidden print-card"><div className="bg-blue-900 text-white px-4 py-3 flex justify-between"><h3 className="font-bold text-lg">{m.nombre}</h3><span>{lista.length} trabajos · {duracionTxt(horas)}</span></div><p className="text-xs text-gray-500 px-4 pt-3">Horario: {horarioTxt(m.horario)}</p>{lista.length === 0 ? <p className="p-4 text-gray-500">Sin vehiculos asignados.</p> : lista.map((t) => { const fin = calcularFinLaboral(t.fechaInicio, t.duracionHoras, t.horario || m.horario); return <article key={t.id} className="p-4 border-t space-y-2"><div className="flex justify-between gap-3"><h4 className="font-bold">{t.matricula || "Sin matricula"} - OR {t.numeroOR || "Sin OR"}</h4><span className={`text-xs border px-2 py-1 rounded-full font-bold ${badge(t.estado)}`}>{estadoTxt(t.estado)}</span></div><p className="text-sm text-gray-600">{t.cliente || "Sin cliente"} {t.marcaModelo ? `- ${t.marcaModelo}` : ""}</p><div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm"><p>Inicio: <strong>{fechaTexto(t.fechaInicio)}</strong></p><p>Finalizacion estimada: <strong>{fechaTexto(fin)}</strong></p><p>Tiempo estimado: <strong>{duracionTxt(t.duracionHoras)}</strong></p></div><p className="text-sm rounded bg-amber-50 border border-amber-200 px-3 py-2 text-amber-900"><strong>Al finalizar:</strong> avisar al asesor por chat o presencialmente.</p>{t.notas && <p className="text-sm">Notas: {t.notas}</p>}<div className="flex gap-2 no-print"><select value={t.estado || "pendiente"} onChange={(e) => cambiarEstado(t.id, e.target.value)} className="border rounded px-2 py-1 text-sm"><option value="pendiente">Pendiente</option><option value="en_proceso">En proceso</option><option value="terminado">Terminado</option></select><button onClick={() => borrarTrabajo(t.id)} className="bg-red-500 text-white px-3 py-1 rounded text-sm">Eliminar</button></div></article>})}</div>})}</div></section>
    </div>
  </main>
}
