...
const horaTexto = (v) => { const f = toDate(v); return f && !isNaN(f) ? `${pad(f.getHours())}:${pad(f.getMinutes())}` : "" }
...
export default function AgendaMecanicosPage() {
  ...
  async function asignarTrabajo(e) {
    ...
    } catch (e) { console.error(e); mostrar("error", "Error", "No se ha podido asignar el vehículo.") }
    finally { setGuardando(false) }
  }
  async function cambiarEstado(id, estado) { if (estado === "terminado") { mostrar("ok", "Trabajo terminado", "Trabajo marcado como terminado."); await crearNotificacion("Trabajo terminado", "Un trabajo de agenda se ha marcado como terminado.", "agenda", "/agenda-mecanicos") } await updateDoc(doc(db, "agenda_mecanicos", id), { estado, actualizadoEn: serverTimestamp() }); await cargarDatos() }
  ...
  const cargaPorMecanico = (m) => {
    const lista = trabajosDelDia.filter((t) => mismoMecanico(t, m))
    const horas = lista.reduce((a, t) => a + Number(t.duracionHoras || 0), 0)
    return { lista, horas }
  }
  const trabajosEnHora = (m, h) => trabajosDelDia.filter((t) => {
    if (!mismoMecanico(t, m)) return false
    const ini = toDate(t.fechaInicio); const fin = calcularFinLaboral(t.fechaInicio, t.duracionHoras, t.horario || m.horario) || ini
    if (!ini || !fin) return false
    const tramoIni = h * 60, tramoFin = tramoIni + 60
    return minutosDia(ini) < tramoFin && minutosDia(fin) > tramoIni
  })
