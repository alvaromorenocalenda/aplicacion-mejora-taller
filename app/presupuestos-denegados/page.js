...
export default function PresupuestosDenegadosPage() {
  ...
  const handleDeleteChat = async (trabajoId) => {
    ...
    if (!confirm("¿Seguro que quieres borrar el chat asociado a este presupuesto denegado? El trabajo NO se borrará.")) return;
    try {
  ...
  async function handleReopenPresupuesto(checklistId) {
    ...
    try {
      ...
      setCuestionarios((prev) => prev.filter((c) => c.id !== checklistId)); setChecklists((prev) => prev.filter((c) => c.id !== checklistId)); setRecambios((prev) => prev.filter((c) => c.id !== checklistId)); setChats((prev) => prev.filter((c) => (c.cuestionarioId || c.id) !== checklistId));
      alert("Presupuesto reabierto correctamente.");
    } catch (err) { console.error(err); alert("Error al reabrir: " + err.message); }
  }
  ...
  const renderItem = (c, tipo) => {
    ...
    return <div key={`${tipo}-${c.id}`} className="flex flex-col sm:flex-row sm:justify-between sm:items-center bg-red-100 p-4 mb-2 rounded gap-3"><div><p className="font-medium">{trabajoLabel(c)}</p><p className="text-xs text-gray-600">Denegado/fecha: {fechaTexto(fechaFiltro(c))}</p></div><div className="grid grid-cols-2 gap-2 sm:flex sm:space-x-2 sm:whitespace-nowrap"><button onClick={() => router.push(`/chat-trabajo/${encodeURIComponent(cuestionarioId)}`)} className="px-3 py-1 bg-fuchsia-600 text-white rounded hover:bg-fuchsia-700">Chat</button><button onClick={() => handleDeleteChat(cuestionarioId)} className="px-3 py-1 bg-rose-700 text-white rounded hover:bg-rose-800">Eliminar chat</button><button onClick={() => router.push(rutaVer)} className="px-3 py-1 bg-blue-500 text-white rounded hover:bg-blue-600">Ver</button><button onClick={() => handleReopenPresupuesto(cuestionarioId)} className="px-3 py-1 bg-green-500 text-white rounded hover:bg-green-600">Reabrir</button><button onClick={() => confirmAndDeleteAll(cuestionarioId)} className="px-3 py-1 bg-red-500 text-white rounded hover:bg-red-600">Eliminar</button></div></div>;
  };
  const renderChat = (c) => { const cuestionarioId = c.cuestionarioId || c.id; return <div key={`chat-${c.id}`} className="flex flex-col sm:flex-row sm:justify-between sm:items-center bg-fuchsia-50 border border-fuchsia-200 p-4 mb-2 rounded gap-3"><div><p className="font-medium">{trabajoLabel(c)}</p><p className="text-xs text-gray-600">Última actividad: {c.tieneChatGuardado ? fechaTexto(c.updatedAt || c.createdAt) : "Sin mensajes todavía"}</p><p className="text-sm text-gray-700 truncate max-w-xl">{c.lastMessage || c.lastText || "Chat asociado al trabajo"}</p></div><div className="flex flex-wrap gap-2"><button onClick={() => router.push(`/chat-trabajo/${encodeURIComponent(cuestionarioId)}`)} className="px-3 py-2 bg-fuchsia-600 text-white rounded hover:bg-fuchsia-700">Abrir chat</button><button onClick={() => handleDeleteChat(cuestionarioId)} className="px-3 py-2 bg-rose-700 text-white rounded hover:bg-rose-800">Eliminar chat</button></div></div>; };

  return <main className="max-w-5xl mx-auto p-6 space-y-8">
    <div className="flex justify-between items-center gap-3 flex-wrap"><div><h1 className="text-3xl font-bold">Presupuestos Denegados</h1><p className="text-gray-600">Por defecto se muestran los denegados del mes actual.</p></div><button onClick={() => router.push("/dashboard")} className="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600">← Volver al Dashboard</button></div>
    <div className="flex flex-wrap gap-3 bg-white rounded-xl shadow p-4 items-end sticky top-3 z-10"><button onClick={() => irASeccion("cuestionarios-denegados")} className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Ir a cuestionarios cliente</button><button onClick={() => irASeccion("checklists-denegadas")} className="px-4 py-2 bg-yellow-500 text-white rounded hover:bg-yellow-600">Ir a checklist denegadas</button><button onClick={() => irASeccion("recambios-denegados")} className="px-4 py-2 bg-blue-800 text-white rounded hover:bg-blue-900">Ir a recambios denegados</button><button onClick={() => irASeccion("chats-denegados")} className="px-4 py-2 bg-fuchsia-600 text-white rounded hover:bg-fuchsia-700">Ir a chats asociados</button><label className="ml-auto text-sm font-semibold">Filtro por mes<input type="month" value={mes} onChange={(e) => setMes(e.target.value)} className="block border rounded px-3 py-2 mt-1" /></label><button onClick={() => setMes(mesActual())} className="px-4 py-2 bg-green-600 text-white rounded">Mes actual</button><button onClick={() => setMes("")} className="px-4 py-2 bg-gray-600 text-white rounded">Ver todos</button></div>
