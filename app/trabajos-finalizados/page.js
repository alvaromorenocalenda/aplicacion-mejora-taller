...
export default function TrabajosFinalizadosPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const idsFinalizados = new Set([
        ...cq.map((x) => x.id),
        ...checklistsCompletas.map((x) => x.cuestionarioId || x.id),
        ...recambiosCompletos.map((x) => x.cuestionarioId || x.id),
      ]);
      const chatsAsociados = chatsCompletos.filter((x) => idsFinalizados.has(x.cuestionarioId || x.id));
  ...
  const renderItem = (c, tipo) => {
    ...
    return <div key={`${tipo}-${c.id}`} className="flex flex-col sm:flex-row sm:justify-between sm:items-center bg-green-100 p-4 mb-2 rounded gap-3">
      ...
      <div className="grid grid-cols-3 gap-2 sm:flex sm:space-x-2 sm:whitespace-nowrap">
  ...
  return <main className="max-w-5xl mx-auto p-6 space-y-8">
    ...
    <div className="flex flex-wrap gap-3 bg-white rounded-xl shadow p-4 items-end sticky top-3 z-10">
      <button onClick={() => irASeccion("cuestionarios-finalizados")} className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Ir a cuestionarios cliente</button>
      ...
      <button onClick={() => irASeccion("chats-finalizados")} className="px-4 py-2 bg-fuchsia-600 text-white rounded hover:bg-fuchsia-700">Ir a chats asociados</button>
