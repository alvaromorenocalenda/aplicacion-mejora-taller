...
function buildDateTime(fecha, hora, fallbackHora) {
  const f = fecha || todayIso();
  const h = hora || fallbackHora;
...
export default function InformesChatsPage() {
  ...
  async function buscarMensajes(e) {
    ...
    if (Number.isNaN(desde.getTime()) || Number.isNaN(hasta.getTime())) {
      setError("Revisa las fechas y horas introducidas.");
      return;
    }

    if (hasta < desde) {
      setError("La fecha/hora final no puede ser anterior a la inicial.");
      return;
    }
    ...
    try {
      ...
      async function getTrabajo(trabajoId) {
        ...
        try {
          ...
          // ignorar y probar cuestionario
        }

        if (!info.matricula && !info.numeroOR && !info.nombreCliente) {
          try {
            const cuestionarioSnap = await getDoc(
              doc(db, "cuestionarios_cliente", trabajoId)
            );
      ...
      if (enlaceIndice) {
        setIndexUrl(enlaceIndice);
        setError("Firebase necesita crear un índice para poder generar este informe.");
      } else {
        setError("No se ha podido generar el informe. Revisa la consola del navegador por si Firebase muestra el enlace del índice.");
      }
    } finally {
      setLoading(false);
    }
  ...
  if (!permitido) {
    return (
      <main className="max-w-4xl mx-auto p-6 space-y-4">
        <h1 className="text-3xl font-bold">Informes Chats</h1>
        <div className="bg-white p-6 rounded shadow text-red-700 font-semibold">
  ...
  return (
    <main className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      <header className="flex justify-between items-center flex-wrap gap-3 no-print">
        <div>
          ...
          <p className="text-gray-600">
            Informe de mensajes escritos en los chats por rango de fecha y hora.
          </p>
        </div>
      ...
      <form
        onSubmit={buscarMensajes}
        className="bg-white p-4 rounded shadow grid grid-cols-1 md:grid-cols-6 gap-3 items-end no-print"
      >
        <div>
        ...
        <div>
          <label className="block text-sm font-semibold mb-1">Hora desde</label>
          <input
            type="time"
            value={horaDesde}
        ...
        <div>
          <label className="block text-sm font-semibold mb-1">Hora hasta</label>
          <input
            type="time"
            value={horaHasta}
        ...
        <div>
          <label className="block text-sm font-semibold mb-1">Buscar texto</label>
          <input
            type="text"
            value={busqueda}
            onChange={(e) => setBusqueda(e.target.value)}
            placeholder="Matrícula, OR, usuario..."
            className="w-full border rounded px-3 py-2"
          />
        </div>
        <button
      ...
      <section className="bg-white rounded shadow overflow-hidden">
        <div className="p-4 border-b flex justify-between items-center flex-wrap gap-3">
          <div>
            <h2 className="text-xl font-bold">Resultado del informe</h2>
            <p className="text-sm text-gray-600">
              Desde {fechaDesde} {horaDesde} hasta {fechaHasta} {horaHasta} · {filtrados.length} mensajes
            </p>
          </div>
        ...
        {error ? (
          <div className="m-4 p-4 bg-red-50 text-red-700 rounded border border-red-200 space-y-3">
            <p>{error}</p>
            {indexUrl ? (
              <a
                href={indexUrl}
                ...
                className="inline-block px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 font-semibold"
              >
        ...
        {loading ? (
          <p className="p-4 text-gray-600">Cargando mensajes…</p>
        ) : filtrados.length === 0 ? (
          <p className="p-4 text-gray-600">No hay mensajes en ese rango.</p>
        ) : (
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-gray-100">
                <tr>
                  ...
                  <th className="text-left p-3 border-b">Hora</th>
                  <th className="text-left p-3 border-b">Matrícula</th>
                  <th className="text-left p-3 border-b">Nº OR</th>
              ...
              <tbody>
                {filtrados.map((r) => (
                  <tr key={`${r.trabajoId}-${r.id}`} className="hover:bg-gray-50 align-top">
                    ...
                    <td className="p-3 border-b whitespace-pre-wrap min-w-[360px]">
                      {r.texto}
                      <div className="text-[11px] text-gray-400 mt-1 no-print">
                        ID trabajo: {r.trabajoId}
                      </div>
                    </td>
                  </tr>
                ))}
